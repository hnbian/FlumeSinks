...
public class HBase2Sink extends AbstractSink implements Configurable, BatchSizeSupported {
    ...
    public void configure(Context context) {
        ...
        serializerContext.putAll(context.getParameters());
    ...
    public Status process() throws EventDeliveryException {
        ...
        try {
            ...
            if (i == 0) { //判断如果i 为0 说明 该批次为空
                status = Status.BACKOFF;
                sinkCounter.incrementBatchEmptyCount();
            } else {
                if (i < batchSize) { //如果i < batch Size
                ...
                logger.info("HBase2Sink putEventsAndCommit " + i + " events");
                putEventsAndCommit(mutations);
            }
            txn.commit();
        } catch (Throwable e) {
            try {
    ...
    private void putEventsAndCommit(final List<Mutation> mutations) throws Exception {
        privilegedExecutor.execute((PrivilegedExceptionAction<Void>) () -> {
            table.mutate(mutations);
            table.flush();
            return null;
        });
        sinkCounter.addToEventDrainSuccessCount(mutations.size());
    }
